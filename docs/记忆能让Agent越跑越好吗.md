...
**会，但真正的飞跃，取决于回路里有没有一个判定源——一个能告诉 agent "对不对 / 该抽什么"的外部权威（标准答案、人类反馈、或带评分标准的裁判）。**
...
- 一旦把判定源质量的反馈存进记忆，同一套机制立刻**突飞猛进**（我们的关键指标从 1/7 跳到 7/7）。
...
还有一个诊断指标后面会反复用到——**真召回失败数**：标准答案里某条，和我们抽出的**任何**条目都毫无重叠，即"根本没抽出来"。它和"抽了但对不齐"是两回事，区分它们是整个故事的关键。
...
于是把所有漏抽的标准答案捞出来，按前面说的真召回失败分类。发现一个清晰的规律：**漏掉的大部分（约 3/4）是"真召回失败"——根本没抽出来**。而这些没抽出来的东西高度同质：
...
它们**都不是"设备+部件+指标+数值"这种定量三元组**，而是离散文字、流程、待确认类的要求。
...
> **先说清一个贯穿全文的诊断指标——「离散文字类探针」。** 我们从 §3 那批"真召回失败"里挑出 7 个代表词：`真空度测量 / 外观颜色 / 喷涂 / 油漆 / 地脚螺栓 / 与甲方确认 / 细化设计`，然后看抽取结果里出现了几个。它专门探测"离散文字/流程类要求这一类，到底抽没抽到"——是"任务规格类教训"学没学会的探针。**7/7 = 这 7 类离散文字要求全抓到；1/7 = 只碰到 1 类。**（它和覆盖率不同：覆盖率是整体一对一匹配率，探针只盯这一类最能区分成败的漏项。）
...
真召回失败从 27 塌到 1，之前漏的离散文字类全抓到了。**证明这是提示词/任务定义问题，改得动**——不是方法无效。

**但这里有个必须讲清的反差：真召回失败从 27 崩到 1（几乎归零），覆盖率却只从 0.642 涨到 0.715（+0.07）。为什么？**

因为这是**两把不同的尺，量的是两件事**：

- **真召回失败是一把"松"尺**（确定性关键词重叠）：只问"这条标准答案和我抽出的**任何**一条有没有词面重叠？"。改了提示词后，模型**开始去抽**那些离散文字类原文了，于是几乎每条标准答案都能找到一条"沾边"的抽取项——真召回失败自然崩到接近 0。它衡量的是**"我们有没有不再整片忽略某类内容"**。
- **覆盖率是一把"严"尺**（LLM 裁判，一对一语义对齐）：它要求裁判为每条标准答案找到一条**能干净配对、判为真匹配**的抽取项。"沾边"不算数。

反差就来自这里：**"开始抽了"（沾边）≠ "抽得刚好能一对一对上标准答案"**。新抽出的离散文字条目虽然覆盖了话题，但常常：
1. **粒度和标准答案对不齐**——模型把一段要求拆/并的方式和标准答案不同，裁判判成"部分"而非干净命中（在残余漏抽里，这类"粒度/对齐问题"占了主要部分，真正的零重叠只剩 1 条）；
2. **多抽带来精确率代价**——抽取条数从 83 涨到 126（+43 条），其中一些是重复或范围没界定好的，配不上一条独立标准答案，于是**精确率从 ~0.85 降到 0.698**，覆盖率也就抬不了那么多。

**换句话说：+0.07 的覆盖率其实"低估"了真实进步。** 我们已经从"整类视而不见"变成了"基本都抽到、只是粒度还没对齐裁判的口径"——真召回失败 27→1 才是这一步质变的真实刻度；覆盖率受限于一对一对齐的严格度和多抽的精确率代价，只反映了其中"能干净配对"的那部分。（残余的粒度问题是评分口径问题，可继续优化，但它不再是"召回失败"。）
...
我们让**一个大模型对比"抽取结果"和"标准答案"，产出可迁移的改进反馈**（"你系统性漏抽了离散文字/流程类要求，应该把非定量要求也抽出来、映射进指标字段……"——是**规则性指引，不是照抄答案**），把这条反馈写进 AgentCore Memory，然后用**基线提示词**重新抽取、**只从记忆注入这条反馈**。关键约束：**标准答案只进"检查器"，绝不进抽取器**。

| | 覆盖率 | 精确率 | F1 | 真召回失败 | 离散文字类 |
|---|---|---|---|---|---|
| 基线 | 0.642 | – | – | 27 | 1 / 7 |
...
> ⚠️ 别误读成"记忆比提示词强"：这里 0.813（记忆）> 0.715（提示词）**主要是 run 间方差**——同一份文档、同样设置，覆盖率本就在 0.55–0.81 间跳（见局限）。两者装的是**同一条教训**，结论是"**放提示词还是放记忆都能兑现增益**"，不是"记忆更优"。真正稳健、能区分成败的信号是**离散文字类探针都到了 7/7**、真召回失败都降到个位数。
...
结果：跑几轮、沉淀十几条规则，那根"探针"（离散文字类抓到几个）**纹丝不动，钉死在 1 / 7**。

模型学不到"该抽离散文字类"——因为它只看自己的产出，没有任何外部信号告诉它"这些也算数"。它能蒸馏出的只是通用文风规则（"复合指标要拆分"），碰不到真问题，甚至因过度强调"拆分"而**帮倒忙**（把句子切碎、拉低精确率）。
...
| 记忆里存的内容 | 来源 | 离散文字类探针 |
|---|---|---|
| 模型对自己产出的自省规则 | 无判定源 | **1 / 7**（学不到）|
| 大模型对比标准答案产出的反馈 | 有判定源 | **7 / 7**（全学会）|

**因果就此锁定：记忆的注入机制一直是好的。之前记忆在这条教训上没起作用，是因为"无判定源自省"够不着"该抽什么"这类问题——它产出的是通用文风规则，而非指名道姓的漏项。**

```mermaid
flowchart TD
  Q["记忆没让 agent 变好"] --> A{"是机制问题<br/>还是内容问题?"}
  A --> B["实验B: 同一记忆机制<br/>存'无判定源自省规则'<br/>→ 探针 1/7"]
  A --> C["实验C: 同一记忆机制<br/>存'判定源人类反馈'<br/>→ 探针 7/7"]
  B --> R["结论: 机制是好的<br/>瓶颈是内容质量<br/>而内容质量取决于判定源"]
  C --> R
...
前面反复出现"判定源"。它是整篇的关键，值得单独讲清。

**判定源 = 一个能告诉 agent「对不对 / 该抽什么」的外部权威。**（这一概念源自软件测试中的"判定基准"。）核心特征：**它提供的信息，从输入本身推不出来**——必须由外部权威给。

它有很多形式，标准答案只是其中一种：

| 判定源形式 | 它告诉 agent 什么 | 现实中常见吗 |
|---|---|---|
| **标准答案** | 这篇「应该」抽出哪些 | 有标注集时 |
| **人类反馈** | 「这类也要抽」/「这条错了」 | 最常见（客服纠正、专家审阅）|
| **大模型对比标准答案/规范做检查** | 按规范/答案判每条对不对、漏了哪类 | 很常见（reviewer 模型、LLM-as-judge）|
| **下游验证** | 抽出的参数拿去用、报错了 → 抽错了 | 有闭环系统时 |

**为什么判定源是记忆有效性的前提？** 模型看着「外观颜色须与甲方确认」，光凭"文档原文 + 自己的输出"永远推不出"这句该成为一条记录"——因为在它的认知里这就不是指标。只有一个外部权威说"这也算一条"，它才知道。**这个「该抽什么」的信号，藏在判定源里，不在输入里。**

由此分出**两类教训**，对判定源的依赖完全不同：

- **任务规格类**（该抽什么 / 什么粒度 / 什么算一条）：**必须有判定源**。这是实验里那条决定成败的教训。
- **源覆盖 / 一致性类**（文档提到 X 你没抽、重复了、格式错了）：对照**原文**即可发现，**没判定源也能学一点**。

> **这里说的"对照原文自我批评"实验是什么？** 我们额外做过一个 Level-2 验证循环：抽完之后，让一个**批评者只看「原文 + 自己刚才的输出」、不看标准答案**，指出"文档里出现了 X、但你没抽 X"，再据此重抽一遍。它**无标准答案**也救回了 **4–10 条**漏项——因为"文档提到了却没抽"这种事，光对照原文就能发现。**但它救不回"该抽离散文字类要求"那种教训**——那需要外部告诉它"这类也算数"，是判定源才有的信号。所以：**无判定源也能学，但只能学"源覆盖/一致性"这一档，天花板低得多。**

> 一句话：**记忆不产生新知识，它只搬运。搬运什么，取决于回路里的判定源。** 这也解释了 AWS 博客那套 self-learning 为什么有效——它吃的是人类/下游反馈这个判定源，不是模型的自我空想。
...
**核心原则：记忆是载体，不是老师。它负责"捕获并复用"反馈，但反馈本身必须由判定源提供。**

具体到工程实践：

1. **先确保回路里有判定源。** 没有"对错/目标"的外部信号，记忆再多也只在原地打转。判定源不必是完整标准答案——**人类的一次纠正、少量标注样本、一个带评分标准的裁判**都行。（这也解释了 AWS 博客那套 self-learning 为什么有效：它吃的是**人类/下游反馈**，不是模型的自我空想。）

2. **记忆里存"具体的、对照答案/原文的批评"，不要存"抽象文风规则"。** "你漏了真空度测量这一段"是有用的；"复合指标要拆分"是空洞的，还会诱发过度拆分反噬。

3. **判定源可以只用一次。** 我们用标准答案做了**一次**错误分析，把结论沉淀成一条可复用记忆——之后就能无标注地跑。用少量监督撬动长期收益，是最划算的用法。

4. **区分你想学的是哪类教训：**
   - **任务规格类**（该抽什么、什么粒度、什么算一条）——**必须有判定源**，从"文档+自己的输出"推不出来。
   - **源覆盖 / 一致性类**（文档提到 X 你没抽、重复了、格式错了）——对照原文即可，**没判定源也能学一点**（我们另一个"对照原文自我批评"的实验，无标准答案也救回了一部分漏项，只是天花板低得多）。
...
- **对应 LangChain 的 [Loop Engineering](https://www.langchain.com/blog/the-art-of-loop-engineering) 框架**：本项目正是其 **Level 4「爬山循环」**——每次运行 → 反思 → 改写辅助配置（记忆）→ 下轮更好。我们的发现给这个框架补了一个前提条件：**爬山需要一个能指明"上坡方向"的判定源，否则只是在原地随机游走。**
...
| 回路 | 教训来源 | 教训放哪 | 覆盖率 | 精确率 | F1 | 真召回失败 | 离散文字探针 |
|---|---|---|---|---|---|---|---|
| 基线（无记忆） | — | — | 0.642 | – | – | 27 | 1/7 |
| 自省记忆 | 模型自省（无判定源） | 记忆 | 0.561 | – | – | 18* | **1/7** |
...
（*自省记忆真召回失败=18 低于基线 27 是"过度拆分"的噪声——切碎的细条碰巧 token 重叠上；但离散文字探针 1/7 证明它真正该学的一条都没学会。所以看探针，别看这个 18。）
（自动脚本版 = `run_experiment_llm_feedback.py`：固定提示词的检查器冷启动对比标准答案产出反馈→写记忆→基线提示词重抽，hands-off 可复现。与交互式版结论一致：cov 0.789、精确率 0.882、F1 0.833、真召回失败 2、探针 6/7。）
...
[合并] 你是「规则库维护器」。把「现有规则集 + 本轮新规则」合并成一份规范规则集
       （去重、≤15 条、每条可操作）。
```
> 注意：这两步**只看模型自己的输出**，没有任何标准答案/外部反馈——这正是它学不到"该抽离散文字类"的原因。

**④ 实验 C 的 LLM 检查器提示词（有判定源，对比标准答案产出反馈）**
```
...
```
> 关键差别：④ 的输入里**有标准答案**（判定源），③ 没有。同样是"让 LLM 产出记忆"，一个学得到、一个学不到。
...
**实验 B 产出的记忆（无判定源自省，坏例子）**——15 条抽象规则里，多条在强调"拆分/独立成条"：
```
5. 多部件拆分规则：一条原文描述多个部件时按部件逐条拆分…
6. 复合指标拆分规则：同一原文含多个指标维度时按指标名称逐条拆分…
9. 阀门/仪表清单抽取规则：每项分别抽「规格」与「数量」为两条独立记录…
```
> 问题一目了然：全是**通用文风/粒度规则**，没有一条说"该把离散文字/流程类要求也抽出来"——因为模型对着自己的输出，根本不知道自己漏了这一类。而且过度强调"拆分"还诱发了过度拆分反噬。

**实验 C 注入的记忆（有判定源反馈，好例子）**——交互式版本写进记忆的反馈：
```
【质检反馈·务必逐条应用】
...
```
> 效果：注入这条自动反馈后，cov 0.789、精确率 0.882、F1 0.833、真召回失败 2、探针 6/7——与人写反馈基本一致。**这证明"对比标准答案产出反馈"这一步可以由模型 hands-off 自动完成，不依赖某个人在场。**
