...
**会，但真正的飞跃，取决于回路里有没有一个 判定源——一个能告诉 agent "对不对 / 该抽什么"的外部权威（标准答案、人类反馈、或带评分标准的裁判）。**
...
- 一旦把 判定源 质量的反馈存进记忆，同一套机制立刻**突飞猛进**（我们的关键指标从 1/7 跳到 7/7）。
...
于是把所有漏抽的标准答案捞出来，按前面说的 真召回失败 分类。发现一个清晰的规律：**漏掉的大部分（约 3/4）是"真召回失败"——根本没抽出来**。而这些没抽出来的东西高度同质：
...
**但这里有个必须讲清的反差：真召回失败 从 27 崩到 1（几乎归零），覆盖率却只从 0.642 涨到 0.715（+0.07）。为什么？**

因为这是**两把不同的尺，量的是两件事**：

- **真召回失败 是一把"松"尺**（确定性关键词重叠）：只问"这条标准答案 和我抽出的**任何**一条有没有词面重叠？"。改了提示词后，模型**开始去抽**那些离散文字类原文了，于是几乎每条标准答案 都能找到一条"沾边"的抽取项——真召回失败 自然崩到接近 0。它衡量的是**"我们有没有不再整片忽略某类内容"**。
...
> ⚠️ 别误读成"记忆比提示词强"：这里 0.813（记忆）> 0.715（提示词）**主要是 run 间方差**——同一份文档、同样设置，覆盖率本就在 0.55–0.81 间跳（见局限）。两者装的是**同一条教训**，结论是"**放提示词还是放记忆都能兑现增益**"，不是"记忆更优"。真正稳健、能区分成败的信号是**离散文字类探针都到了 7/7**、真召回失败 都降到个位数。
...
| 记忆里存的内容 | 来源 | 离散文字类探针 |
|---|---|---|
| 模型对自己产出的自省规则 | 无 判定源 | **1 / 7**（学不到）|
| 大模型对比标准答案产出的反馈 | 有 判定源 | **7 / 7**（全学会）|

**因果就此锁定：记忆的注入机制一直是好的。之前记忆在这条教训上没起作用，是因为"无 判定源 自省"够不着"该抽什么"这类问题——它产出的是通用文风规则，而非指名道姓的漏项。**
...
flowchart TD
  ...
  B --> R["结论: 机制是好的<br/>瓶颈是内容质量<br/>而内容质量取决于 判定源"]
  C --> R
...
**为什么 判定源 是记忆有效性的前提？** 模型看着「外观颜色须与甲方确认」，光凭"文档原文 + 自己的输出"永远推不出"这句该成为一条记录"——因为在它的认知里这就不是指标。只有一个外部权威说"这也算一条"，它才知道。**这个「该抽什么」的信号，藏在 判定源 里，不在输入里。**

由此分出**两类教训**，对 判定源 的依赖完全不同：

- **任务规格类**（该抽什么 / 什么粒度 / 什么算一条）：**必须有 判定源**。这是实验里那条决定成败的教训。
- **源覆盖 / 一致性类**（文档提到 X 你没抽、重复了、格式错了）：对照**原文**即可发现，**没 判定源 也能学一点**。

> **这里说的"对照原文自我批评"实验是什么？** 我们额外做过一个 Level-2 验证循环：抽完之后，让一个**批评者只看「原文 + 自己刚才的输出」、不看标准答案**，指出"文档里出现了 X、但你没抽 X"，再据此重抽一遍。它**无标准答案**也救回了 **4–10 条**漏项——因为"文档提到了却没抽"这种事，光对照原文就能发现。**但它救不回"该抽离散文字类要求"那种教训**——那需要外部告诉它"这类也算数"，是 判定源 才有的信号。所以：**无 判定源 也能学，但只能学"源覆盖/一致性"这一档，天花板低得多。**

> 一句话：**记忆不产生新知识，它只搬运。搬运什么，取决于回路里的 判定源。** 这也解释了 AWS 博客那套 self-learning 为什么有效——它吃的是人类/下游反馈这个 判定源，不是模型的自我空想。
...
**核心原则：记忆是载体，不是老师。它负责"捕获并复用"反馈，但反馈本身必须由 判定源 提供。**

具体到工程实践：

1. **先确保回路里有 判定源。** 没有"对错/目标"的外部信号，记忆再多也只在原地打转。判定源 不必是完整标准答案——**人类的一次纠正、少量标注样本、一个带评分标准的裁判**都行。（这也解释了 AWS 博客那套 self-learning 为什么有效：它吃的是**人类/下游反馈**，不是模型的自我空想。）

2. **记忆里存"具体的、对照答案/原文的批评"，不要存"抽象文风规则"。** "你漏了真空度测量这一段"是有用的；"复合指标要拆分"是空洞的，还会诱发过度拆分反噬。

3. **判定源 可以只用一次。** 我们用标准答案做了**一次**错误分析，把结论沉淀成一条可复用记忆——之后就能无标注地跑。用少量监督撬动长期收益，是最划算的用法。

4. **区分你想学的是哪类教训：**
   - **任务规格类**（该抽什么、什么粒度、什么算一条）——**必须有 判定源**，从"文档+自己的输出"推不出来。
   - **源覆盖 / 一致性类**（文档提到 X 你没抽、重复了、格式错了）——对照原文即可，**没 判定源 也能学一点**（我们另一个"对照原文自我批评"的实验，无标准答案也救回了一部分漏项，只是天花板低得多）。
...
- **对应 LangChain 的 [Loop Engineering](https://www.langchain.com/blog/the-art-of-loop-engineering) 框架**：本项目正是其 **Level 4「爬山循环」**——每次运行 → 反思 → 改写辅助配置（记忆）→ 下轮更好。我们的发现给这个框架补了一个前提条件：**爬山需要一个能指明"上坡方向"的 判定源，否则只是在原地随机游走。**
...
|---|---|---|---|---|---|---|---|
| 基线（无记忆） | — | — | 0.642 | – | – | 27 | 1/7 |
| 自省记忆 | 模型自省（无 判定源） | 记忆 | 0.561 | – | – | 18* | **1/7** |
...
（*自省记忆 真召回失败=18 低于基线 27 是"过度拆分"的噪声——切碎的细条碰巧 token 重叠上；但离散文字探针 1/7 证明它真正该学的一条都没学会。所以看探针，别看这个 18。）
（自动脚本版 = `run_experiment_llm_feedback.py`：固定提示词的检查器冷启动对比标准答案 产出反馈→写记忆→基线提示词重抽，hands-off 可复现。与交互式版结论一致：cov 0.789、精确率 0.882、F1 0.833、真召回失败 2、探针 6/7。）
...
**④ 实验 C 的 LLM 检查器提示词（有 判定源，对比标准答案产出反馈）**
```
...
**实验 B 产出的记忆（无 判定源 自省，坏例子）**——15 条抽象规则里，多条在强调"拆分/独立成条"：
```
...
**实验 C 注入的记忆（有 判定源 反馈，好例子）**——交互式版本写进记忆的反馈：
```
【质检反馈·务必逐条应用】
