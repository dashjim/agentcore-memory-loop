...
> **先说清一个贯穿全文的诊断探针——「离散文字类试金石」。** 我们从 §3 那批"真召回失败"里挑出 7 个代表词：`真空度测量 / 外观颜色 / 喷涂 / 油漆 / 地脚螺栓 / 与甲方确认 / 细化设计`，然后看抽取结果里出现了几个。它专门探测"离散文字/流程类要求这一类，到底抽没抽到"——是"任务规格类教训"学没学会的试金石。**7/7 = 这 7 类离散文字要求全抓到；1/7 = 只碰到 1 类。**（它和覆盖率不同：覆盖率是整体一对一匹配率，试金石只盯这一类最能区分成败的漏项。）
...
> ⚠️ 别误读成"记忆比提示词强"：这里 0.813（记忆）> 0.715（提示词）**主要是 run 间方差**——同一份文档、同样设置，覆盖率本就在 0.55–0.81 间跳（见局限）。两者装的是**同一条教训**，结论是"**放提示词还是放记忆都能兑现增益**"，不是"记忆更优"。真正稳健、能区分成败的信号是**离散文字类试金石都到了 7/7**、真召回失败 都降到个位数。
...
结果：跑几轮、沉淀十几条规则，那根"试金石"（离散文字类抓到几个）**纹丝不动，钉死在 1 / 7**。
...
> **这里说的"对照原文自我批评"实验是什么？** 我们额外做过一个 Level-2 验证循环：抽完之后，让一个**批评者只看「原文 + 自己刚才的输出」、不看标准答案**，指出"文档里出现了 X、但你没抽 X"，再据此重抽一遍。它**无标准答案**也救回了 **4–10 条**漏项——因为"文档提到了却没抽"这种事，光对照原文就能发现。**但它救不回"该抽离散文字类要求"那种教训**——那需要外部告诉它"这类也算数"，是 oracle 才有的信号。所以：**无 oracle 也能学，但只能学"源覆盖/一致性"这一档，天花板低得多。**
...
| 回路 | 教训来源 | 教训放哪 | 覆盖率 | 精确率 | F1 | 真召回失败 | 离散文字试金石 |
|---|---|---|---|---|---|---|---|
| 基线（无记忆） | — | — | 0.642 | – | – | 27 | 1/7 |
...
（*自省记忆 真召回失败=18 低于基线 27 是"过度拆分"的噪声——切碎的细条碰巧 token 重叠上；但离散文字试金石 1/7 证明它真正该学的一条都没学会。所以看试金石，别看这个 18。）
