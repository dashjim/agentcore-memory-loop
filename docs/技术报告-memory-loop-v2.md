...
**观察到的结论（修复评分后，措辞审慎）**：在这组**单文档、串行、小样本**运行中——① **未观察到记忆对覆盖率的可信增益**：V2 mem(~0.62) 不高于 nomem(~0.66) 且随轮次下降；冷启动 Episodic 对照（§3.3）后段略高（~0.75）但被 nomem 的大方差(0.545–0.756)淹没，同样不显著。**先前（评分 bug 未修）"记忆使覆盖率上升"很大程度是覆盖率高估的假象。** ② 记忆**机制**层面成立：consolidation 合并出**一份规范规则集（13 条）**（非 V1 版本堆叠）；Episodic 冷启动=0 已确认（修正 V1 基线污染）。③ **抽取路径 token 较 V1 custom 降约 10–20 倍**，但 **judge token 才是成本大头**（~9万–12万，此前未计）。④ 结论强度受限于单文档/小样本/单 judge，严格验证需多文档盲测 + 多 seed + CI（review 的未来工作）。
