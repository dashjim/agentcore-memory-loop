...
- **① 同文档、有/无记忆**：在这份文档+这套记忆提示词下，**未观察到覆盖率(召回)增益**——V2 mem(~0.62) 不高于 nomem(~0.66)；冷启动 Episodic（§3.3）后段略高(~0.75)但被 nomem 大方差(0.545–0.756)淹没、不显著。**注意：这是"该设置下无增益"，不等于"记忆方法无效"**。旧版"记忆使覆盖率上升"主要是评分 bug 高估的假象。
- **①′ 优化验证（§3.5）**：按 §3.4 诊断改抽取/记忆提示词后重跑——**过度拆分被成功抑制（记录/原文 1.92→1.16）**，但**覆盖率/F1 没因此上升（反略降）**，易漏类别召回仅 0→1/10。→ 更新诊断：低覆盖率**主因不是过度拆分**，更可能是**基础抽取上限 + GT/judge 粒度**；一轮提示词优化未能翻转"无增益"，严格验证需转向多文档盲测/多 seed/CI（非继续调 prompt）。
...
### 3.5 优化验证：按 §3.4 的诊断改提示词后重跑（是否"提示词问题"？）

§3.4 提出两个可优化假设：(A) 基础抽取盲区（检测/环境/受压元件类别漏抽）——在抽取 system-prompt 显式补类别指引；(B) 记忆规则诱发过度拆分——在 reflect/consolidation 提示词里**抑制过度拆分、默认"一原文一记录"**。据此加了 `_OPT_EXTRACT_ADDON`（补 检测/环境/受压元件/焊接 类别 + "禁过度拆分"）与 `_REFLECT_SYS_OPT`/`_CONSOLIDATE_SYS_OPT`，`run_ablation(opt=True)` 重跑 **nomem×2 + mem×3**（落 `runs_v2_opt.db`，脚本 `scripts/run_experiments_opt.py`），同一文档、其余不变。

| 运行 | 覆盖率(召回) | 精确率 | F1 | 抽取条数 |
|---|---|---|---|---|
| opt-nomem#1 | 0.667 | 0.854 | 0.749 | 96 |
| opt-nomem#2 | 0.545 | 0.859 | 0.667 | 78 |
| opt-mem#1（冷,canonical空） | 0.667 | 0.854 | 0.749 | 96 |
| opt-mem#2 | 0.520 | 0.877 | 0.653 | 73 |
| opt-mem#3 | 0.520 | 0.877 | 0.653 | 73 |

**对比（确定性启发式，取各库"记忆累积最充分"的一轮：基线 mem#4 vs opt-mem#3）**：

| 指标 | 基线 mem | opt mem | 说明 |
|---|---|---|---|
| 记录/唯一原文（过度拆分度） | **1.92** | **1.16** | ✅ 假设(B)的**机制**验证成立——改提示词把过度拆分压下去了（102→73 条） |
| 覆盖率(LLM judge) | 0.561 | 0.520 | ❌ 没升反略降；opt-mem 内部 0.667(冷)→0.520→0.520，规则累积后**下降** |
| F1 | — | 0.653 | 与基线量级相当，未见增益 |
| 易漏类别召回(检测/环境/受压/焊) | 0/10 | **1/10** | ❌ 显式补类别提示词只救回 1 条，系统性漏抽基本没解决 |

**观察（如实，这是一次有信息量的负面结果）**：
- **假设(B)"过度拆分"被证伪为覆盖率的杠杆**：改提示词确实把过度拆分从 1.92 压到 1.16（机制层面按预期生效），但覆盖率**不升反降**。说明之前那些"过度拆出的细条"在一对一评分下**反而在捞部分 GT 命中**；抑制拆分让条目变粗变少，匹配到的 GT 也变少。→ 过度拆分**不是**低覆盖率的成因。
- **假设(A)"类别盲区靠提示词可补"只得到极弱支持**：显式加检测/环境/受压元件指引，易漏类别召回仅 0→1/10。→ 这些系统性漏抽**比"提示词措辞"更顽固**，更可能是**模型在该文档上的抽取上限**或 **GT/judge 粒度对齐**问题，而非一句 prompt 能补。
- **对研究问题的更新**：一轮针对性提示词优化**没有**把"无记忆增益"翻转为增益。这**既不证明记忆有效、也不证明无效**，但它**更新了 §3.4 的诊断**——短板主因**不是过度拆分**（改了反而更差），更可能是基础抽取上限 + GT/judge 粒度。要继续验证，需按 §5 的最小可行重做（多文档盲测、多 seed、CI、人工校准 GT/judge 粒度），而非继续调抽取提示词。

### 3.6 决定性实验：oracle 是记忆有效性的前提（→ 最终结论见 blog）

§3.5 的 opt-v1 未翻转"无增益"，据此把 §3.4 诊断深挖，做了三个环环相扣的实验（完整叙述见综合版 blog [`记忆能让Agent越跑越好吗.md`](记忆能让Agent越跑越好吗.md)）。关键判据用 **recall_fail（真召回失败：与任何抽取项零重叠）** 和 **散文类试金石**（真空度测量/外观颜色/油漆/与甲方确认等散文/流程类要求抓到几个，共 7 个探针）。

| 回路 | 教训来源 | 教训放哪 | 覆盖率 | 精确率 | F1 | recall_fail | 散文试金石 |
|---|---|---|---|---|---|---|---|
| 基线 nomem | — | — | 0.642 | – | – | 27 | 1/7 |
| GT-free 自省记忆(V2 mem) | 模型自省(无 oracle) | 记忆 | 0.561 | – | – | 18* | **1/7** |
| opt-v2 nomem | GT 错误分析 | 提示词 | 0.715 | 0.698 | 0.707 | 1 | 7/7 |
| **人类反馈记忆** | GT 错误分析(当反馈) | **记忆** | **0.813** | 0.813 | **0.813** | 2 | **7/7** |

**误差分析（§3.4 深化）**：漏抽主体（~3/4）是**真召回失败**，且集中在**散文/流程/待确认类要求**（「外观颜色须与甲方确认」「真空度测量报告」「外部油漆等工序」等）——不符合"设备+部件+指标+数值"定量三元组，被 schema 系统性跳过。opt-v2 不改 schema，只显式指示"非定量要求也抽、映射进 指标名称/指标特征"+ 点名易漏类别，recall_fail 27→1、试金石 1/7→7/7。

**决定性对照（第 2 行 vs 第 4 行）**：注入路径完全相同（都是记忆），唯一差别是内容——**GT-free 自省规则→试金石钉死 1/7（学不到任务规格）；把同一条 GT 错误分析当人类反馈写进记忆→7/7、cov 0.642→0.813**。

**结论**：记忆的注入机制一直是好的；之前 mem 无增益，坏的是"GT-free 自省"产出的抽象规则（还诱发过度拆分反噬）。**记忆能否让 agent 越跑越好，取决于回路里有没有 oracle（GT/人类反馈/rubric-judge）；记忆是好的载体，不是好的老师。** 两类教训：任务规格类（必须 oracle）vs 源覆盖/一致性类（GT-free 也能学一点，见 blog 的 reflection 实验）。

> *自省记忆 recall_fail=18 低于基线 27 是过度拆分噪声（细条碰巧 token 重叠上）；试金石 1/7 才是真相——真正该学的一条没学会。
