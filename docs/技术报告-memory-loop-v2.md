...
**观察到的结论（修复评分后，措辞审慎；"同文档"与"泛化"分开）**：
- **① 同文档、有/无记忆**：在这份文档+这套记忆提示词下，**未观察到覆盖率(召回)增益**——V2 mem(~0.62) 不高于 nomem(~0.66)；冷启动 Episodic（§3.3）后段略高(~0.75)但被 nomem 大方差(0.545–0.756)淹没、不显著。**注意：这是"该设置下无增益"，不等于"记忆方法无效"**——很可能是记忆规则偏"过度拆分"或评分粒度所致（§3.4 误差分析 + 优化空间）。旧版"记忆使覆盖率上升"主要是评分 bug 高估的假象。
- **② 泛化到其他文档**：**本轮未测**（全在同一文档），不下任何泛化结论。
- **③ 记忆机制成立**：consolidation 合并出**一份规范规则集（13 条）**（非 V1 版本堆叠）；Episodic 冷启动=0 已确认（修正 V1 基线污染）。
- **④ 成本**：抽取路径较 V1 custom 降约 10–20 倍，但 **judge token 才是大头**（~9万–12万，此前未计）。
- **⑤ 评估**：加了 **精确率/F1**（不止覆盖率），严格验证仍需多文档盲测 + 多 seed + CI（review 未来工作）。
...
> **评估口径（据 review）**：覆盖率=**召回**(匹配 GT/GT总数)；本节新增**精确率**(匹配上的抽取项/抽取总数，惩罚过度抽取)与 **F1**(召回与精确率的调和平均)；准确率=匹配项的**字段级正确率**（另一维度）。单看覆盖率会漏掉"过度抽取"的代价，故以 F1 综合看（P/R/F1 全量见 §3.4 误差分析）。

**观察（如实，措辞审慎；结论按"同文档"与"泛化"分开）**：
- **【同文档 · 有/无记忆】覆盖率(召回)未见记忆提升**：nomem ≈ 0.68/0.64（均值 ~0.66）；mem = 0.659/0.675/0.585/0.561（均值 ~0.62）。**在这份文档、这套记忆提示词下，mem 未高于 nomem。**
- **准确率**两组相当（~0.82–0.88）。
- **抽取条数**：mem 随记忆累积上升（90→93→95→102）而覆盖率没升——**很可能是记忆规则大量强调"拆分/独立成条"导致过度拆分**（抽更多细条、精确率下降、但没覆盖更多不同 GT）。**这指向"记忆提示词 + 评分粒度"的问题，而非证明"记忆方法无效"**（详见 §3.4 误差分析：漏抽 GT 的 pattern + 是否过度拆分）。
- **成本口径（修正）**：**judge token（~8.5万–12万）反而大于抽取路径 token（~3.1万–6.3万）**，单次真实总成本 ~12万–18万。仍远低于 V1 custom 的 96 万，但此前"~5万/次"是**仅抽取路径**、不含 judge/后台。
- **⚠️ 与旧版对比**：旧版（评分 bug 未修）报"记忆使覆盖率 0.77→0.86 上升"；修复"跨块重复匹配"后该提升消失——**先前的正向结论很大程度是覆盖率高估的假象**（review 的 P0）。
- **两类结论务必分开（客观）**：
  1. **同文档、有/无记忆**：以上是**有效的对照结果**——本设置下未见覆盖率增益；但**能否归因于"记忆无用"尚不能下**，因为可能是提示词（过度拆分）或评分粒度所致（§3.4 分析 + 优化空间）。
  2. **泛化到其他文档**：**本轮未测**（全在同一文档上），故**不对泛化下任何结论**。
- **仍成立的工程结论**：consolidation 把逐轮经验合并成**一份规范规则集**（§3.2）——记忆**机制**层面的改进，独立于"是否提升抽取质量"。
...
### 3.3 Episodic 干净对照（冷启动 + 修复后评分 + 异步等待）
为纠正 V1 episodic 的两大问题（评分 bug、未重置），用**全新 actorId 冷启动**（运行前确认 `/episodes/{actor}` 记录数=0）、**修复后一对一评分**、每轮后**轮询等 AgentCore 异步提炼完成**再下一轮，重做 episodic（extractor_ep harness，同一文档）：

| 运行 | 覆盖率 | 准确率 | 抽取条数 | 已注入 episode 数 | 抽取路径tok | judge tok |
|---|---|---|---|---|---|---|
| nomem#1 | 0.756 | 0.856 | 101 | — | 41,100 | 98,496 |
| nomem#2 | 0.545 | 0.789 | 97 | — | 49,378 | 110,311 |
| episodic#1(冷) | 0.667 | 0.857 | 87 | 0（提炼未完成） | 23,295 | 98,258 |
| episodic#2 | 0.634 | 0.845 | 88 | 0（仍未完成） | 18,597 | 91,352 |
| episodic#3 | 0.756 | 0.827 | 113 | 2 | 27,425 | 107,864 |
| episodic#4 | 0.740 | 0.843 | 106 | 3 | 27,545 | 103,656 |

**观察（如实）**：
- **有微弱正向迹象但不显著**：episodic 后两轮（记忆真正注入后，~0.75）高于前两轮冷启动（~0.65）；但 **nomem 自身就在 0.545–0.756 剧烈波动**，与该差异同量级。**样本 2/4、无重复无 CI、方差巨大 → 不能判定 episodic 有增益**（nomem#1=0.756 与 episodic#3 相同）。
- **异步提炼慢且不稳**：ep#1 后等待超时仍 0 条；4 次运行只产出 **3 条 episode**（并非每 session 都产出一条）。→ 依赖异步提炼的召回在"连续跑"下时机难保证。
- **成本**：episodic 抽取路径更省（~1.8万–2.7万，原生技能单次输出干净），但 **judge token 仍达 ~9万–11万**（成本大头）。
  > ⚠️ **口径澄清**："judge token"是**我方 LLM-as-judge 评分**那步的 token（每 run 都要评分，与模式无关），从 `invoke_harness` 响应流的 `metadata.usage` 事件读出累加——**不是 Episodic 提炼的 token**。**Episodic 异步提炼的真实成本 API 不返回、本项目未知/未计入**（局限）。
- **结论**：与 V2 custom 一致——**在本单文档/小样本/单 judge 设置下，未得到"记忆提升覆盖率"的可信证据**；episodic 的迹象需按 review 的多文档盲测 + 多 seed + CI 才能验证。冷启动=0 已确认（修正了 V1 的基线污染问题）。

---

### 3.4 误差分析：漏抽了什么、是否过度拆分、优化空间
> 方法：确定性启发式（**非 LLM judge**——评分时 harness 运行时 502 频发，改用可复现的 token 覆盖启发式）。判据：GT 原文的关键 token 被某抽取项覆盖 <50% 记为漏抽；"记录数 / 唯一原文数"衡量过度拆分。对象 mem#4(102条,cov0.561) vs nomem#1(94条,cov0.683)。

**(A) 漏抽的 pattern —— 主要是"基础抽取提示词"的盲区，与记忆无关**
- mem 漏 ~24、nomem 漏 ~27；**其中 19 条两组共漏**，各自独有仅 3 / 8 条。
- 漏抽 GT 集中在：**`检测项目(RT/PT/UT 无损检测)`、`安装环境(温/风/雪/地震)`、`主要受压元件`、`部件字段为空的 GT`**。
- → 这些**系统性漏抽 mem/nomem 高度重叠**，说明是**基础抽取 prompt/schema 没覆盖好这些类别**（NDE 检测标准、环境条件、受压元件），**不是记忆造成的**。**优化空间**：在抽取 system-prompt / schema 里显式补这些类别的抽取指引。

**(B) mem 过度拆分 —— 是"记忆规则"的问题**
- mem：102 条来自仅 **53 个唯一原文 → 1.92 条/原文**；一句"内罐所有对接接头采用…"被拆成 **8 条**，"接口尺寸…"拆成 7 条。
- nomem：94 条来自 **70 个唯一原文 → 1.34 条/原文**（拆分明显更少）。
- → 累积的 canonical 规则**过度强调"按指标类型逐条拆分 / 独立成条"**，使 agent 把一句原文切成多条细记录；在**一对一评分**下这些细条各自只覆盖 GT 的一部分、匹配不上完整 GT，**拉低精确率与覆盖率**。**优化空间**：调 consolidation/reflect 提示词，**抑制过度拆分、让抽取粒度对齐 GT 的"一原文一记录"粒度**。

**(C) 结论（客观）**：本组"记忆未提升覆盖率"的**主因是提示词与评分粒度**——(A) 基础抽取盲区（两模式共有）+ (B) 记忆规则诱发的过度拆分——**而非"记忆方法本身无效"**。这些都是**可优化**的（改抽取 prompt 补类别、改记忆规则抑制过度拆分、或让评分粒度与抽取粒度一致后重测）。**在修好这些之前，不宜对"记忆有效性"下结论。**
